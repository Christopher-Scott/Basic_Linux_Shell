...
int main(int argc, char **argv){
    ...
    pid_t pid;
    int status;
    int fd[2], in = 0, out = 0, append = 0, save_in, save_out;
        ...
        *(line + (chars_read - 1)) = '\0';  // get rid of newline character
        ...
        // -------------- SETUP REDIRECTION -----------------
            ...
            if(current->output != NULL && current->append != NULL){ // can't have both
                ...
                if((out = open(buf, O_WRONLY | O_CREAT | O_TRUNC, S_IWUSR)) == -1){
                    fprintf(stderr, "%s\n", strerror(errno));
                }
                ...
                if((out = open(buf, O_WRONLY | O_CREAT | O_APPEND, S_IWUSR)) == -1){
                    fprintf(stderr, "%s\n", strerror(errno));
                }
                save_out = dup(STDOUT_FILENO);
                dup2(out, STDOUT_FILENO);
            ...
            // ---------------- CLEAN UP ------------------
            if(in != 0){ // Restore stdin
                dup2(save_in, STDIN_FILENO);
                close(in);
                in = 0;
            }
            if(out != 0){ // Restore stdout
                dup2(save_out, STDOUT_FILENO);
                close(out);
                out = 0;
            }
            if(append != 0){ // Restore stdout
                dup2(save_out, STDOUT_FILENO);
                close(append);
                append = 0;
            }
